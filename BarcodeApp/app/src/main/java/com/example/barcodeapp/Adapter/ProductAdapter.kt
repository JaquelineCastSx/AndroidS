...
class ProductAdapter(val products:List<Product>):RecyclerView.Adapter<ProductAdapter.ViewHolder>(){
    class ViewHolder(item: View) : RecyclerView.ViewHolder(item){
        val name = item.findViewById<TextView>(R.id.txt_product_name)
        val price = item.findViewById<TextView>(R.id.txt_product_price)
        val image = item.findViewById<ImageView>(R.id.imgV_product_image)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.price.text = product.priceFormatter
